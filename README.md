...
This implementation allows for using different environments. It's not restricted to OpenAI gym environments. If you want to attach the project to another environment rather than that provided by gym, all you have to do is to inherit from the base class `BaseEnv` class in `envs/base_env.py`, and implement all the methods in a plug and play fashion. You also have to add the name of the new environment class to the dictionary in `A2C.py\env_name_parser()` method.
